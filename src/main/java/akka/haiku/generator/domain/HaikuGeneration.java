...
public record HaikuGeneration(String userInput,
                              Long generatedAt,
                              HaikuGenerationStatus status,
                              Optional<Haiku> haiku,
                              Optional<Image> image) {

  public static HaikuGeneration empty() {
    return new HaikuGeneration("***********", null, STARTED, Optional.empty(), Optional.empty());
  }

  public HaikuGeneration withImageUrl(String imageUrl) {
    return new HaikuGeneration(userInput, generatedAt, IMAGE_GENERATED, haiku, Optional.of(new Image(imageUrl)));
  }

  public HaikuGeneration withUserInput(String userInput) {
    return new HaikuGeneration(userInput, generatedAt, status, haiku, image) ;
  }

  public HaikuGeneration withHaiku(Haiku haiku) {
    return new HaikuGeneration(userInput, Instant.now().toEpochMilli(), HAIKU_GENERATED, Optional.of(haiku), image);
  }
  ...
  public HaikuGeneration accepted() {
    return new HaikuGeneration(userInput, generatedAt, ACCEPTED, haiku, image);
  }

  public HaikuGeneration toxicityDetected() {
    return new HaikuGeneration(userInput, Instant.now().toEpochMilli(), TOXICITY_DETECTED, haiku, Optional.of(new Image("static/img/censored.png")));
  }

  public HaikuGeneration negativityDetected() {
    return new HaikuGeneration(userInput, Instant.now().toEpochMilli(), NEGATIVITY_DETECTED, haiku, Optional.of(new Image("static/img/censored.png")));
  }

  public HaikuGeneration validated() {
    return new HaikuGeneration(userInput, generatedAt, VALIDATED, haiku, image);
  }

  public HaikuGeneration timedOut() {
    return new HaikuGeneration(userInput, Instant.now().toEpochMilli(), TIMED_OUT, haiku, Optional.of(new Image("static/img/time-is-up.png")));
  }
